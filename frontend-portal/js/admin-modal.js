 ...
 */
import { escapeHtml, ModalManager, showToast } from "./security-utils.js";

function fmtDate(str) {
  if (!str) return "—";
  const d = new Date(str);
  if (isNaN(d)) return escapeHtml(str) || str;
...
async function loadData() {
  ...
  const loadingMsg =
    '<p class="mx-status-msg loading">▸ Accessing secure node...</p>';
  ...
  try {
    ...
    if (res.status === 403) {
      usersBody.innerHTML =
        '<p class="mx-status-msg error">▸ ACCESS DENIED: Insufficient permissions.</p>';
      return;
    }
    ...
    const errorMsg =
      '<p class="mx-status-msg error">▸ CRITICAL: Node fetch failed.</p>';
  ...
  try {
    ...
    if (!res.ok) {
      ...
      if (typeof showToast === "function")
        showToast(data.error || "Request rejected.", "error");
      return;
    }
    ...
    if (typeof showToast === "function") {
      showToast(
        actionType === "delete-user" ? "User purged." : "Access revoked.",
        "success",
      );
    }
  } catch (err) {
